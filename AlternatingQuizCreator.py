...
#Question setup
#The script will randomly select and move numQuestionsPerFolder questions from each folder
# questionFolders. It will then commit and push the questions online for the student to complete.
numQuestionsPerFolder = 2
questionFolders = []

#Folder Setup
# Each folder should contain a set of questions for each group of students
# There should be one folder appended for each roster path provided.
...
nameGroups = []
for rosterPath in rosterPaths:
    with open(rosterPath) as f:
        nameGroups.append( f.read().splitlines() )
    ...
    for i in range(len(nameGroups)):
        ...
        for name in names:
            ...
            if os.path.exists(f'{repoPath}/{folderName}'):   #Check to see if the project has been cloned before
                ...
                filePaths = []
                path, fileExtension = questionFolders[i]
